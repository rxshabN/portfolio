...
export default function Skills() {
  const { ref: sectionInViewRef } = useSectionInView("Skills", 0.5);
  ...
  // useEffect(() => {
  //   if (sectionInViewRef && typeof sectionInViewRef === "function") {
  //     sectionInViewRef(intersectionRef);
  //   }
  // }, [sectionInViewRef, intersectionRef]);

  const combinedRef = (node: HTMLElement) => {
    // Pass the element to both refs
    if (intersectionRef) intersectionRef(node);
    if (sectionInViewRef) sectionInViewRef(node);
  };
